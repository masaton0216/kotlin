...
private val defaultApiVersion = ApiVersion.LATEST_STABLE

private val apiVersionCheck = ConsistencyCheck<KotlinSourceSet, ApiVersion>(
    name = "language version",
    getValue = { sourceSet ->
        sourceSet.languageSettings.apiVersion?.let { parseApiVersionSettings(it) } ?: defaultApiVersion
    },
    leftExtendsRightConsistently = { left, right -> left >= right },
    consistencyConditionHint = "The API version of the dependent source set must be greater than or equal to that of its dependency."
)

private val unstableFeaturesCheck = ConsistencyCheck<KotlinSourceSet, Set<LanguageFeature>>(
    ...
    consistencyConditionHint = "The dependent source set must enable all unstable language features that its dependency has."
)
...
internal val defaultSourceSetLanguageSettingsChecker = SourceSetConsistencyChecker(
    listOf(languageVersionCheck, apiVersionCheck, unstableFeaturesCheck)
