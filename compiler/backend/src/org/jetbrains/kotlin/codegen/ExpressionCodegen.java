...
public class ExpressionCodegen extends KtVisitor<StackValue, StackValue> implements LocalLookup, BaseExpressionCodegen {
    ...
    @NotNull
    private StackValue generate754EqualsForNullableTypes(
            @NotNull KtExpression left,
            @NotNull KtExpression right,
            @NotNull IElementType opToken,
            @Nullable StackValue pregeneratedSubject,
        ...
        if (state.getLanguageVersionSettings().getApiVersion().compareTo(ApiVersion.KOTLIN_1_1) >= 0) {
            return StackValue.operation(Type.BOOLEAN_TYPE, v1 -> {
                generate754EqualsForNullableTypesViaIntrinsic(v1, opToken, pregeneratedSubject, left, left754Type, right, right754Type);
                return Unit.INSTANCE;
            });
        }
        else {
            return StackValue.operation(Type.BOOLEAN_TYPE, v1 -> {
                generate754EqualsForNullableTypes(v1, opToken, pregeneratedSubject, left, left754Type, right, right754Type);
                return Unit.INSTANCE;
            });
        }
    }
